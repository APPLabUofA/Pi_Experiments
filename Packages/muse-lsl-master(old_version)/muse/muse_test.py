...
class Muse():
    ...
    def _handle_eeg(self, handle, data):
        ...
        # last data received
        if handle == 35:
            if tm != self.last_tm + 1 or os.path.isfile("/home/pi/Experiments/Packages/muse-lsl-master/muse/Test_EEG.csv") == True:
                missing_sample_count += 1
                print(missing_sample_count)
                print("missing sample %d : %d" % (tm, self.last_tm))
                if missing_sample_count == 1:
                    stop_eeg = [0]
                    np.savetxt("/home/pi/Experiments/Packages/muse-lsl-master/muse/Pause_EEG.csv", (stop_eeg), delimiter=',',fmt="%s")
                    missing_sample_count = 0
            self.last_tm = tm
