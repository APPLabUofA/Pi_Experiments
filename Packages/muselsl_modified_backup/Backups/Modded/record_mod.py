...
from .stream_mod import find_muse
...
def record(duration, filename=None, dejitter=False, data_source="EEG", partnum=None, device=None, filename_path=None, exp_loc=None):
    ...
    filename_save = ("/home/pi/research_experiments/Experiments/" + exp_loc + "/Data/Muse/LSL_Data/" + partnum + "_" + filename_path + ".csv" )
    ...
    stop_eeg = [0]
    np.savetxt("/home/pi/research_experiments/Stop_EEG2.csv", (stop_eeg), delimiter=',',fmt="%s")
    ...
    while os.path.isfile("/home/pi/research_experiments/Stop_EEG2.csv") == True:
        try:
    ...
    data.to_csv(filename_save, float_format='%.3f', index=False)

    print('Done - wrote file: ' + filename_save + '.')
...
def record_direct(duration, address, filename=None, backend='auto', interface=None, name=None):
    ...
    recording.to_csv(filename_save, float_format='%.3f')
    print('Done - wrote file: ' + filename_save + '.')
