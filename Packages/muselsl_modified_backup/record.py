...
def record(duration, filename=None, dejitter=False, data_source="EEG", exp=None):
    ...
    if not exp:
        raise Exception('Pleace provide and experiment name/folder, in the format ''--exp exp_name''.')
    if not filename:
        filename = '/home/pi/Pi_Experiments/' + exp + '/Data/Muse/LSL_Data/'

    ### check if exp folder exists###
    if not os.path.isdir('/home/pi/Pi_Experiments/' + exp):
        raise Exception(r'Experiment folder /home/pi/Pi_Experiments/' + exp + ' doesn''t exist!')
    ...
    stop_eeg = [0]
    np.savetxt("/home/pi/Pi_Experiments/Stop_EEG2.csv", (stop_eeg), delimiter=',',fmt="%s")
    ...
    while os.path.isfile("/home/pi/Pi_Experiments/Stop_EEG2.csv") == True:
        try:
    ...
    partnum = '001'
    while os.path.isfile(filename + partnum + '_' + data_source + '_' + exp + '.csv') == True:
        if int(partnum) >= 9:
            partnum = '0' + str(int(partnum) + 1)
        else:
            partnum = '00' + str(int(partnum) + 1)

    filename = filename + partnum + '_' + data_source + '_' + exp + '.csv'
