...
for par in range(1,9): # 1,2,3,4,5,6,7,8
    if par != 2:       # 1,3,4,5,5,6,7,8
        for Vid_Num in range(1,2): # 1,2 - Video 1 only, # 1,3 - Video 1 & 2 # 2,3 for Video 2 only
...
#                df1b = df1b.drop(columns='index')
                ...
                export_csv = df1b.to_csv (r'M:\Data\GoPro_Visor\Experiment_1\Video_Times\Dataframe_df1b_Vid_1_00' + str(par) + '.csv', index = None, header=True) #Don't forget to add '.csv' at the end of the path

                export_csv = df1.to_csv (r'M:\Data\GoPro_Visor\Experiment_1\Video_Times\Dataframe_df1_Vid_1_00' + str(par) + '.csv', index = None, header=True) #Don't forget to add '.csv' at the end of the path
