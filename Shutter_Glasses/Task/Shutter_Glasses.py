...
for i_tone in range(len(tones)):
    ...
    if tones[i_tone] == 0:#square
        stim1 = pygame.draw.rect(screen, rect_white , ((x_center-(rect_size/2)),(y_center-(rect_size/2)),rect_size,rect_size))
        trig_type.append(1)
    elif tones[i_tone] == 1:#triangle
        stim1 = pygame.draw.polygon(screen, rect_white , ((x_center,y_center-(rect_size/2)),(x_center-(rect_size/2),y_center+(rect_size/2)),(x_center+(rect_size/2),y_center+(rect_size/2))))
        trig_type.append(2)
    ###draw stim and send trigger### 
    trig_time.append(time.time() - exp_start)
    GPIO.output(pi2trig(int(tones[i_tone])+1),1)
    pygame.display.update([stim1])
    ...
    if tones[i_tone] == 0:#square
        ...
        stim2 = pygame.draw.polygon(screen, rect_black , ((x_center,y_center-(rect_size/2)),(x_center-(rect_size/2),y_center+(rect_size/2)),(x_center+(rect_size/2),y_center+(rect_size/2))))
    pygame.display.update([stim2])
