...
partnum = '001'
device = 'Amp'
filename = 'Shutter_Glasses_Face_Pics_Swapped'
...
scrambled_face_loc = '/home/pi/research_experiments/Experiments/Shutter_Glasses/Images/Faces/'

nature_num_1 = list(range(50))
urban_num_1 = list(range(50))
face_num_1 = list(range(59))
scrambled_face_num_1 = list(range(59))
...
scrambled_face_num_2 = list(range(59))
...
instructions_1 = myfont.render('Press ''W'' when you see a nature scene. ', True, white)
instructions_2 = myfont.render('Press ''S'' when you see an urban scene. ', True, white)
instructions_3 = myfont.render('Press ''Up'' when you see a rightside-up face. ', True, white)
instructions_4 = myfont.render('Press ''Down'' when you see an upside-down face. ', True, white)
instructions_5 = myfont.render('When you are ready to start, press the space bar.', True, white)
...
screen.blit(instructions_4,(x_center-((instructions_4.get_rect().width)/2),y_center + ((instructions_4.get_rect().height)*4)+10))
screen.blit(instructions_5,(x_center-((instructions_5.get_rect().width)/2),y_center + ((instructions_5.get_rect().height)*5)+10))
...
for i_trial in range(len(all_trials)):
    ...
    if all_trials[i_trial][0] == 1:#nature
        ...
        resp_event = pygame.K_w
        trig_type.append(1)
        pic_num.append(all_trials[i_trial][1])
    elif all_trials[i_trial][0]  == 2:#urban
        trial_img = pygame.image.load(urban_loc + 'Urban_' + str(all_trials[i_trial][1]) + '.tif')
        trial_img = pygame.transform.scale(trial_img,(rect_x,rect_y))
        resp_event = pygame.K_s
        trig_type.append(2)
        pic_num.append(all_trials[i_trial][1])
    elif all_trials[i_trial][0]  == 3:#face
        trial_img = pygame.image.load(face_loc + 'Face_' + str(all_trials[i_trial][1]) + '.tif')
        trial_img = pygame.transform.scale(trial_img,(rect_x,rect_y))
        resp_event = pygame.K_UP
        trig_type.append(3)
        pic_num.append(all_trials[i_trial][1])
    elif all_trials[i_trial][0]  == 4:#upside_down face
        trial_img = pygame.image.load(scrambled_face_loc + 'Face_' + str(all_trials[i_trial][1]) + '.tif')
        trial_img = pygame.transform.scale(trial_img,(rect_x,rect_y))
        trial_img = pygame.transform.flip(trial_img,False,True)
        resp_event = pygame.K_DOWN
        trig_type.append(4)
        pic_num.append(all_trials[i_trial][1])
    ...
    key_pressed = 0
    pygame.event.clear()
    while key_pressed == 0 and time.time() - start_resp <= delay:
        event = pygame.event.poll()
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
        elif event.type == pygame.KEYDOWN:
            if event.key == resp_event:
                key_pressed = 1  
    if key_pressed == 1:  
