...
partnum = '001'
device = 'Muse'
filename = 'Baseline_Pet_Therapy_Updated'
...
###send triggers to indicate start of the first block###
exp_start = time.time()
...
###wait for button press to start second block###
key_pressed = 0
pygame.event.clear()
while key_pressed == 0:
    event = pygame.event.wait()
...
the_list = [trig_time,timestamps]
df_list = pd.DataFrame({i:pd.Series(value) for i, value in enumerate(the_list)})
df_list.columns = ['Baseline_Onset_Offset_Time','Timestamps']
