...
baseline_length = 300 ###5 minutes in seconds
...
###send triggers to indicate start of experiment###
exp_start = time.time()
...
###send triggers for start of first three minute segment###
...
###send triggers to indicate the end of the first three minute segment###
trig_time.append(time.time() - exp_start)
timestamp = local_clock()
...
###wait for button press to start the second segment###
key_pressed = 0
pygame.event.clear()
while key_pressed == 0:
    event = pygame.event.wait()
...
###send triggers to indicate the start of the tones for the second three minute segment###
trig_time.append(time.time() - exp_start)
timestamp = local_clock()
...
###play the tones again to indicate the start of the second segment###
pygame.mixer.music.load(tone)
for i_tone in range(3):
    pygame.mixer.music.play()
    while pygame.mixer.music.get_busy() == True:
...
###send triggers to indicate the start of the second three minute segment###
trig_time.append(time.time() - exp_start)
timestamp = local_clock()
...
###send triggers to indicate the end of the second three minute segment###
trig_time.append(time.time() - exp_start)
timestamp = local_clock()
...
###play the tones to indicate the end of the second segment and the experiment###
pygame.mixer.music.load(tone)
for i_tone in range(3):
    pygame.mixer.music.play()
    while pygame.mixer.music.get_busy() == True:
...
if os.path.isfile("/home/pi/research_experiments/Stop_EEG2.csv") == True:
    ...
    os.mknod("/home/pi/research_experiments/Muse_Shutdown.txt")
