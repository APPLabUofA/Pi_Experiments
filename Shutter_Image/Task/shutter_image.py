...
###define our screen boundaries###
x_min = 0
y_min = 0
x_pos = 0
y_pos = 0
x_dir = 1 ###start by moving fixation to the right###

x_lines = y_height/float(rect_y)
if x_lines == 0: ###scanning will end on the right
    stop_scan = [y_height - rect_y, x_width - rect_x]
else: ###scanning will end on the left
    stop_scan = [y_height - rect_y, 0]

###let's determine how long to scan each line###
###assume we want to scan the image for 30 minutes###
pic_dur = 1*60*1000 ###30 minutes to scan whole picture
pixel_time = ((pic_dur)/((x_lines*(x_width-rect_x))+(y_height-rect_y)))/1000.0 ###ms per pixel
min_draw_time = (0.0048*((x_lines*(x_width-rect_x))+(y_height-rect_y)))
des_draw_time = (pixel_time*((x_lines*(x_width-rect_x))+(y_height-rect_y)))

print(min_draw_time)
print(des_draw_time)

###setting pic_dur to 1 minute results in a 400x300 image taking 3 minutes...
###seems like it takes around 4.8ms to redraw everything on the screen (fill white, draw text, draw new fixation)
