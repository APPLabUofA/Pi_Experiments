...
stop_eeg = [0]
np.savetxt("/home/pi/Experiments/Flanker/Stop_EEG2.csv", (stop_eeg), delimiter=',',fmt="%s")

while os.path.isfile("/home/pi/Experiments/Flanker/Data/Amp/LSL_Data/%s_comb_lsl_data_flanker.csv"%(partnum)) == True:
    partnum = '00'+str(int(partnum)+1)
    if int(partnum) > 9:
        partnum = '0' + str(int(partnum))

filename    = "%s_comb_lsl_data_flanker"%(partnum)

default_fname = ("/home/pi/Experiments/Flanker/Data/Amp/LSL_Data/%s.csv" % filename)
...
while os.path.isfile("/home/pi/Experiments/Flanker/Stop_EEG2.csv") == True:
    try:
