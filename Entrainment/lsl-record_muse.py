...
stop_eeg = [0]
np.savetxt("/home/pi/Experiments/N170/Stop_EEG2.csv", (stop_eeg), delimiter=',',fmt="%s")

while os.path.isfile("/home/pi/Experiments/Entrainment/%s_muse_lsl_data_test.csv"%(partnum)) == True:
    partnum = '00'+str(int(partnum)+1)
    if int(partnum) > 9:
        partnum = '0' + str(int(partnum))

filename    = "%s_muse_lsl_data_test"%(partnum)

default_fname = ("/home/pi/Experiments/Entrainment/%s.csv" % filename)
...
while os.path.isfile("/home/pi/Experiments/N170/Stop_EEG2.csv") == True:
    try:
