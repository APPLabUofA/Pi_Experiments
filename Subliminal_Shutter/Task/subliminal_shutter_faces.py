...
instr.append(myfont.render('A face will be presented on each trial.', True, font_colour))
instr.append(myfont.render('At the start of each trial a prompt will tell you to close your eyes.', True, font_colour))
instr.append(myfont.render('Another prompt will tell you to open your eyes and focus on the face.', True, font_colour))
...
instr.append(myfont.render('When you hear a brief tone, press the ''Up'' arrow if you were shown a right-side-up face, or the ''Down'' arrow if you were shown an up-side-down face.', True, font_colour))
...
###distribute our face type across our three conditions###

vis_trials_type = list(np.zeros(int(trials*(1.0/3.0)))+1)
vis_trials_dc = list(np.zeros(int(trials*(1.0/3.0)))+vis_dc)
vis_trials_up_face = list(np.zeros(int(len(vis_trials_type)/2.0))+1)
vis_trials_down_face = list(np.zeros(int(len(vis_trials_type)/2.0))+2)
vis_trials_faces = vis_trials_up_face + vis_trials_down_face
vis_trials = zip(vis_trials_type,vis_trials_dc,vis_trials_faces)

thresh_trials_type = list(np.zeros(int(trials*(1.0/3.0)))+2)
thresh_trials_dc = list(np.zeros(int(trials*(1.0/3.0)))+thresh_dc)
thresh_trials_up_face = list(np.zeros(int(len(thresh_trials_type)/2.0))+1)
thresh_trials_down_face = list(np.zeros(int(len(thresh_trials_type)/2.0))+2)
thresh_trials_faces = thresh_trials_up_face + thresh_trials_down_face
thresh_trials = zip(thresh_trials_type,thresh_trials_dc,thresh_trials_faces)

invis_trials_type = list(np.zeros(int(trials*(1.0/3.0)))+3)
invis_trials_dc = list(np.zeros(int(trials*(1.0/3.0)))+invis_dc)
invis_trials_up_face = list(np.zeros(int(len(invis_trials_type)/2.0))+1)
invis_trials_down_face = list(np.zeros(int(len(invis_trials_type)/2.0))+2)
invis_trials_faces = invis_trials_up_face + invis_trials_down_face
invis_trials = zip(invis_trials_type,invis_trials_dc,invis_trials_faces)
...
for i_trial in range(len(all_trials)):
    ...
    if all_trials[i_trial][2] == 1:
        GPIO.output(pi2trig(temp_face_num+56),1)
    elif all_trials[i_trial][2] == 2:
        trial_img = pygame.transform.flip(trial_img,False,True)
        GPIO.output(pi2trig(temp_face_num+115),1)
...
the_list = [date, thresh_dc, vis_dc, trig_type, resp_time, resp_type, face_num]
df_list = pd.DataFrame({i:pd.Series(value) for i, value in enumerate(the_list)})
df_list.columns = ['Date','Participant Threshold','Visible Threshold','Trigger_Type','Response_Time', 'Response Type', 'Face Number']
df_list.to_csv(filename_part)
