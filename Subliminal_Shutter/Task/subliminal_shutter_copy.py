...
for i_trial in range(len(all_trials)):
    ...
    ###gradually change duty cycle over 2 seconds###
    dc_incr = (all_trials[i_trial][1])/20.0

    for i_dc in range(20):
        crnt_dc = dc_incr * int(i_dc)
        GPIO.output(pi2trig(int(all_trials[i_trial][0])*10),1)
        left_eye.ChangeDutyCycle(crnt_dc)
        right_eye.ChangeDutyCycle(crnt_dc)
        time.sleep(0.05)
        GPIO.output(pi2trig(255),0)
    ...
    ###slowly decrease dc###
    for i_dc in range(20):
        crnt_dc = all_trials[i_trial][1] - (dc_incr * int(i_dc))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    ...
                    GPIO.output(pi2trig(int(all_trials[i_trial][0])*10+7),1)
                    resp_time.append(time.time()-start_time)
                    key_pressed = 'DOWN'
                    resp_type.append(key_pressed)
            pygame.event.pump()
...
the_list = [date, thresh_dc, trig_type, resp_time, resp_type]
df_list = pd.DataFrame({i:pd.Series(value) for i, value in enumerate(the_list)})
df_list.columns = ['Date','Participant Threshold','Trigger_Type','Response_Time', 'Response Type']
df_list.to_csv(filename_part)
